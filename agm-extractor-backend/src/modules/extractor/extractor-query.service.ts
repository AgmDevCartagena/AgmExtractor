...
export class ExtractorQueryService {
  ...
  private agregarUltimaActuacion<
    T extends {
      actuaciones?: { fechaActuacion: Date | null; actuacion: string | null }[];
    },
  >(proceso: T) {
  ...
  async getDataForScheduledTask(
    ...
    try {
      ...
      this.logger.error(
        `Error al obtener datos para la tarea programada: ${errorMessage}`,
      );
      throw new HttpException(
        `Fallo al obtener datos: ${errorMessage}`,
  ...
  async getRadicadoTasks(pagination: PaginationQueryDto, userId: string) {
    ...
    try {
      ...
      throw new HttpException(
        `Fallo al obtener tareas: ${errorMessage}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getDataForRadicadoTask(pagination: PaginationQueryDto, userId: string) {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Error desconocido';
      this.logger.error(
        `Error al obtener datos de tarea radicado: ${errorMessage}`,
      );
      throw new HttpException(
        `Fallo al obtener datos: ${errorMessage}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
  ...
  async getProcesosUltimaActuacion(userId: string) {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Error desconocido';
      this.logger.error(
        `Error al obtener procesos para exportar última actuación: ${errorMessage}`,
      );
      throw new HttpException(
        `Fallo al obtener datos: ${errorMessage}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
  ...
  async deleteProcesosByTask(
    ...
    try {
      if (modo === 'radicado') {
        ...
        this.logger.log(
          `Procesos eliminados para radar radicado [${taskId}]: ${count}`,
        );
      ...
      this.logger.error(
        `Error al eliminar procesos del radar [${taskId}]: ${msg}`,
      );
      throw new HttpException(
        `Fallo al eliminar procesos: ${msg}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getProcesosParaExportar(userId: string) {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Error desconocido';
      this.logger.error(
        `Error al obtener procesos para exportar: ${errorMessage}`,
      );
      throw new HttpException(
        `Fallo al obtener datos: ${errorMessage}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
  ...
  async getProcesoDetalle(procesoId: string, userId: string) {
    try {
      ...
      const msg = error instanceof Error ? error.message : 'Error desconocido';
      this.logger.error(
        `Error al obtener detalle del proceso [${procesoId}]: ${msg}`,
      );
      throw new HttpException(
        `Fallo al obtener detalle: ${msg}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
