...
export class AdminMetricsService {
    ...
    async overview() {
        ...
        const [
            ...
            this.prisma.procesosJudiciales.count({ where: { fechaDescubrimiento: { gte: last7 } } }),
            this.prisma.procesosJudiciales.count({ where: { fechaDescubrimiento: { gte: last30 } } }),
            this.prisma.procesosJudiciales.count({ where: { detalleExtraido: true } }),
            this.prisma.actuacion.count(),
            this.prisma.executionLog.count({ where: { startedAt: { gte: last24h } } }),
            this.prisma.executionLog.count({ where: { startedAt: { gte: last24h }, status: 'SUCCESS' } }),
            this.prisma.executionLog.count({ where: { startedAt: { gte: last7 } } }),
            this.prisma.executionLog.count({ where: { startedAt: { gte: last7 }, status: 'SUCCESS' } }),
    ...
    async timeseries(metric: 'signups' | 'executions' | 'procesos', range: '7d' | '30d' | '90d') {
    ...
    async performance() {
        ...
        const [total, exito, fallo, corriendo, agg, throughput24h] = await Promise.all([
        ...
        return {
            ...
            procesosPorCorrida: Math.round((agg._avg.procesosEncontrados ?? 0) * 10) / 10,
            throughput24h,
        };
    }
